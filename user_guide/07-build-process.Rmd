...
## Building Package Options
There are a few ways to do this   two ways to set up VisionEval to rebuild a package with localized data.
...
​	• Download the [VisionEval for R 4.1.3 Installer.](https://github.com/VisionEval/VisionEval-Dev/releases/download/beta-release-0.9/VE-3.0-Installer-Windows-R4.1.3_2022-05-27.zip) Unzip it into a convenient directory (i.e., “VisionEval directory”)

​	• Check that VisionEval starts by double-clicking VisionEval.Rproj in the VisionEval directory
...
**3.**Start VisionEval by clicking the VisionEval.Rproj file as you did at the end of Step 1 above (or you may just have left RStudio running).



**4.**Gather the new PUMS .csv files and replace the .csv files in the VisionEval/src/VESimHouseholds/inst/extdata directory

​	• Follow the steps in the online documentation to [pre-process the PUMS data](https://visioneval.org/docs/ve-buildprocess.html#preprocessing-for-visioneval-input-data)
...
**5**.  Configure the RStudio Build Tools (*see images below)*
...
​	• The resulting new version of VESimHouseholds will then be installed with your PUMS data rather than the Oregon defaults and it will be used when you next run VERSPM



For other build methods that may be more appropriate for more experienced users click here:
...
### Introduction to PUMS for VE
- which year of pums, 
   where to get it it, 
- how to use it,  
- what geography, etc. 
- do we look at the data at all beforehand
...
To help with this process, an R script was written with functions that both download the PUMS data and preprocess the data into .CSV files for VisionEval, located here: (script will be pushed to the FHWA repo) https://github.com/RSGInc/VEProcessPUMS
...
### [ nick - move the decennial census pums to the later part of the docu]
...
#### **Most Generic Build Process**

  1. (this is the most sophisticated - big way)
     The most general approach is to start with the source
...
2. Install [[Git for Windows][Git4W]](https://git-scm.com/) or equivalent\
...
4. [Install RTools 4.0](https://cran.r-project.org/bin/windows/Rtools/rtools40.html)

5. [Install RStudio](https://www.rstudio.com/)\
   (a version compatible with RTools40, and with the version of R you're using)

6. Copy or clone the [VisionEval](https://github.com/VisionEval/VisionEval) (or [VisionEval-dev](https://github.com/VisionEval/VisionEval-dev) repository

7. Start RStudio and open the VisionEval-dev.Rproj project file\
   (On most graphic display machines, just double-click it)

8. Execute the `ve.build()` function:\
...
9. Once the build is done, you can get into the runtime environment in several ways:
...
10. If the build is successful, you can make an installer .zip file by doing this:

    ```
    ve.build("installer")
    ```

  11. (package by package way that can be enhanced)
