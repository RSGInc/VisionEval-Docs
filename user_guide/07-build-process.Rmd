...
This chapter describes the package build process for creating or modifying packages and their constituent modules within the VisionEval framework. The build process can be used to incorporate localized or custom data into VE by making a change to a module and then rebuilding it. 

This chapter includes a simple case study example of substituting the default households population data with a locally-specific US Census data Public Use Microdata Sample (PUMS) - a valuable way to get your VE model to reflect local conditions - and then rebuilding the package in VisionEval.

The material is organized to present methods that are applicable to the majority of VE users first. More advanced methods are presented later in this chapter for more advanced R users and model developers.

The generalized approach to updating data within a VE package is set out below. Each of the steps has a detailed set of instructions  within this chapter. The summary of steps are:

1.  Make sure you have a running VisionEval that you can start with RStudio.

    -   Install R (ideally Version 4.1.2).  *note: the VE team will try to keep the versions up to date with the latest software. However, sometimes due to testing compatibility with various packages, the R version for VE lags behind the most current version*
    -   Install RStudio and start it directly. Verify that it is using the R 4.1.2 version
...
6.  Install the package two times.
    ...
    -   The resulting new version of VESimHouseholds will then be installed with your local PUMS data.
...
-   If users are not familiar with [git](https://git-scm.com/downloads) and GitHub, simply download the repository as a zip file (<https://github.com/VisionEval/VisionEval/archive/refs/heads/master.zip>). In either case, the download is very large (over 900MB compressed and 3.4GB uncompressed as git clone). The Zip file download is shown in the figure below.
...
** placeholder: For a more detailed example of processing PUMS with example R scripts, see %%%%LINK TO
...
**helpful hint!** - If you want to test the new files before building it, you may run "Check Package" first.
...
## Case Study: VEPowertrainsandFuels
...
Any of the three build processes described above can be used.
