...
This chapter introduces the option for users of VisionEval models to incorporate third-party synthetic population data produced by software tools. Examples being PopulationSim (https://activitysim.github.io/populationsim/#) and UrbanSim (https://urbansim.com/). This may be particularly valuable for users looking to create parity between population data inputs to VisionEval and existing activity-based models or other travel demand models. The synthetic population is developed in the VESimHouseholds module within VisionEval. This module can be amended to use the third party data. The following sections describe the process for using this data. 
...
1. **For users of existing synthetic data looking to setup a VisionEval model:**

- This process requires the VisionEval is setup with an identical geography as the population data. ***Alert!*** - This may not be a trivial task, as the VisionEval model requires several inputs at specific geographic levels. 

2. **For users of an existing VisionEval model looking to use synthetic data (anticipated to be the more common scenario):**

- This process requires the synthetic data to adopt the same geographies as the VisionEval model.
- The VisionEval model will already have all the data for the various geographies. The synthetic population will have be run with these specific geographies in order to consistently align the data.

The process described below focuses on the second use case where there is an existing operational VisionEval model that will be amended to make use of a new synthetic population that will replace the existing synthetic household population process within VisionEval. The VisionEval model will have an updated module that will use pre-processed data and transform that data to work within the VisionEval datastore specifications. 

This chapter will provide a tool to do the pre-processing as well provide the revised run scripts to run the new synthetic population VisionEval module. This chapter uses a case study in how to design and use a Population Sim model to create a new synthetic population dataset but also include additional household characteristics that haven't been used before in the travel model. These additional characteristics of the household will allow for specific downstream queries to evaluate travel behavior across various household attributes (e.g., race, ethnicity, disability); *note these additional household* *attributes do not influence the travel models directly, but rather are external variables against which travel behavior outcomes can be* *compared to.* The use of PopulationSim is essential to ensure that control totals for these households are held to the same overarching control totals and statistical representation of the true population being modeled.

The remainder of this document following describes the process for using a PopulationSim synthetic population in VisionEval, summarized in the steps below: 

- Creating a third party synthetic population (using PopulationSim in this case study). The PopulationSim can be used to generate not only the required household descriptive fields that are used in VisionEval, but also other desired household characteristics that may be desirable for segment the population during reporting and querying the results.
- Aligning the two models. Specifically the geography of the two models must have a cross-walk to match household characteristics. 
- Third Party Data to VisionEval --> Populating the VisionEval datastore with the third party household and population data. This requires sensitivity to the variable specifications, units, etc. This is expected to be a separate script run as a pre-process to the VisionEval model.
- Revising VisionEval run scripts to run the amended process that excludes the VisionEval SimHouseholds module.

***Note (to be removed when Task 5 is complete) : Task 5.2 sets out the model design for this process. Task 5.3 is designing the working prototype for this process. All references to Tasks, or 5.2 and 5.3 will be removed. **

**This chapter will revised as part of Task 5.3 when the PopSim case study and prototype have been completed.**
...
One of the basic components of VisionEval is the household population being modeled. This population contains key demographic attributes for transportation modeling, such as age, income, employment, and household composition (i.e., the types of persons in each household). A common challenge in transportation modeling is high-dimensional data. As a synthetic population it is important to remain valid with the control totals for each dimensions and within statistical validity of the true population.

Since VisionEval is intended to be lightweight, it only includes a minimum of demographic features associated with key travel behavior parameters. However, it is important to empower users to incorporate additional variables specific to their needs. As such, this documentation also describes the development of a new package, VEPopulationSim, that can utilize an externally-synthesized PopulationSim population instead of relying on the default VESimHouseholds package. *A prototype of* *VEPopulationSim* *will be developed in Task 5.3.* 

This guide is divided into two main sections, one describing VisionEval’s current population synthesis process, and the other describing PopulationSim and use of PopulationSim in VEPopulationSim: 
...
2. [**PopulationSim**](#PopulationSim) inputs, mechanics, and outputs are described for use as a third-party population synthesizer. The VEPopulationSim package, built to host the module that converts the output from PopulationSim synthesizer into a VisionEval dataset, is also described. *This package will be described in greater detail in Task 5.3.* 

**Note**: It is important to clarify that population data and household characteristics appended to the population are for post-processing analysis, and do not introduce changes to travel behavior models.
...
The VESimHouseholds package contains five R code files, consisting of a data preparation step that is invoked when the package is built, and four modules used to synthesize households during a VisionEval model run:

1. **CreateEstimationDatasets.R** - Used during the package build, this set of code formats Public Use Microdata Sample (PUMS) data from comma separate values (CSV) file into a format suitable for use in estimating the population synthesis models and performing population synthesis.
2. **CreateHouseholds.R** - Module to synthesize the base household population data by age group
3. **PredictWorkers.R** - Module to estimate the number of workers in each household by age group
4. **AssignLifeCycle.R** - Module to assign household life cycle variable (LIF_CYC) as defined by 2009 NHTS
5. **PredictIncome.R** - Module to estimate the household income for each household

These individual modules are run sequentially at the beginning of a VisionEval model run, starting with CreateHouseholds, to add relevant information to the household population datastore. The modules need to be run in order, as they have pre-requisite dependencies, such as the AssignLifeCycle module, which requires the number of household workers to be assigned before it can be run in order to determine retired persons. Below is a general summary table of inputs and outputs for each of the R files. An external population synthesizer needs to generate compatible module outputs. 

| R File              | Input files | Output to Datastore During Model Run | Output to Package Data During Package Build |
| ------------------------ | ----------- | ----------------------- | -- |
| CreateEstimationDatasets | VESimHouseholds/inst/extdata/pums_household.csv<br>VESimHouseholds/inst/extdata/pums_persons.csv | Only during build | Hh_df |
...
If the seed data from PUMS is changed in the packages inst/extdata folder, the package will need to be rebuilt. See the [Module Build Process](#ve-buildprocess) for more details.
...
This file, processed when the package is build, reads in the default PUMS data and converts it to an R dataframe and stores it in the VESimHouseholds package data folder. These data in the Hh_df dataframe become the base seed data from which the household populations are synthesized during a model run. To change the PUMS seed data to correspond to the local region, see the [Module Build Process](#ve-buildprocess) for more details.
...
**Note**: The HhType field in this table is **NOT** the same as HhType that is placed in the Datastore and used in subsequent modules.
...
It then carries out the following iterative process to create a set of households that is internally consistent and that matches (approximately) the optional inputs for household size and proportion of single-person households:
...
The life cycle categories are similar, but not the same as, those established for the NHTS. The age categories used in VisionEval models are broader than those used by the NHTS to identify children of different ages. As a result it is not possible for the life cycle designations to reflect child ages as they do in the NHTS. Also, adulthood is determined differently in this module. The NHTS uses age 21 as the threshold age for adulthood. This module uses use 20 as nominal age break for adulthood (the 20-29 age group). Moreover, the module identifies some younger persons to be adults in situations where they are likely to be living independently as adults or emancipated minors. Persons in the 15 to 19 age group are considered adults when there are no older adults (ages 30+) in the household.
...
The following table documents the data columns that are placed in the datastore after running the four modules in the VESimHouseholds package. Each row in the table describes a dataset. The table names and their meanings are as follows:
...
The VEPopulationSim package contains amodule that converts the output from a separate 
PopulationSim synthesizer run into a VisionEval dataset. It also contains several ancillary
...
***Note:*** it is essential that the user has a complete PopulationSim output dataset ready to inform VE before proceeding and both the models share a common geography
...
#### Inputs
1. Settings (`inst/dataprep_settings.R`) - The settings are used to specify file names
...
The ReadPopulationSimOutput module within the VEPopulationSim package will convert the synthetic households and population produced by [PopulationSim](#PopulationSim) into VisionEval households and load them into the datastore. The module and the package are developed using the guidelines provided in the [Developer Documentation](https://rsginc.github.io/VisionEval-Docs/developer.html). There are two key components to this module:
...
- **Main function**:
The ReadPopulationSimOutput module is defined in ve_read_popsim_output_function.R and 
...
## Using VEPopulationSim

### Updating VEPopulationSim

The steps outlined below assumes that the user has access to a PopulationSim setup and the necessary output files (i.e., synthetic households and population within that setup). Some of these steps include editing R scripts (which can be done using any text editor) and others include editing spreadsheets.

***NOTE***: Task 5.3 will focus on coalescing these into one spreadsheet style input file, more typical with other VE inputs, that will be easier for the users to update. 
...
the synthetic_households.csv and synthetic_persons.csv file. Users should follow the guidelines for 
`Inp` component of [Module Specification](https://rsginc.github.io/VisionEval-Docs/ve-estimation.html?q=specifi#module-specifications) when updating or entering new records in the file. Users should not delete any existing attributes that came with the example setup as these attributes are necessary for the VisionEvall models to run successfully.

* Update the VESimHousehold_DataDictionary.csv file with all the attributes that are needed to produce the same output
VESimHousehold package produces and any additional attributes that need to be retrieved from the synthesized households and persons file. Users should follow the guidelines for `Set` component of [Module Specification](https://rsginc.github.io/VisionEval-Docs/ve-estimation.html?q=specifi#module-specifications) when updating or entering new records in the file. The attributes listed in this spreadsheet should not be altered as these are required to successfully run a VisionEval model.

* Update the ve_read_popsim_output_function.R script by updating/defining the relations between the PopulationSim output and VisionEval dataset. Users should follow the guidelines outlined in [Module Function](https://rsginc.github.io/VisionEval-Docs/ve-estimation.html?q=specifi#module-function) to update the function. The objective of this function is to transform synthetic households and persons attributes to VisionEval households and persons attributes.

## Running the model with VEPopulationSim package

To run the model with VEPopulationSim package it is assumed that the users have generated synthetic households and persons file by running the PopulationSim synthesizer and have all the VEModel spatial inputs (bzone or azone) in a format that is spatially consistent with the geography used with the PopulationSim synthesizer. If an user has an existing PopulationSim setup they want to use with this package then additional manual verification steps need to be taken to ensure that the VEModel geography (bzone or azone) aligns with PopulationSim geographies. Additional guidance on this and some helpful tools will be provided in Task 5.3.
