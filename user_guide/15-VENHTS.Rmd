...
- An overview of differences in the estimation results between VE model's estimated using the 2001 and 2017 NHTS. This includes some discussion of the difficulties of working withing the existing VE modules to extract and compare model estimation results given the non-standard model objects structures that are saved and used in model application.

- To test the 2017 NHTS, two versions of the model were run: the original version of the model based on the 2001 NHTS and a new version based on the 2017 NHTS. The comparison used the example Rogue Valley VERSPM model incorporated in VE.

This work builds on some past efforts to explore updates to individual VE modules. For examplem, the Multimodal Module was developed by Portland State University using the 2009 NHTS. That update included creating a new travel demand model design as well as updating the estimation dataset from the 2001 NHTS to 2009 and also updating other datasets to describe land use and transportation supply. While that effort was an extensive effort to research, re-specify, and re-estimate a single module, this work is an attempt at a broader re-estimation without any re-specification of models, in order to see whether that type of update is possible. 

Discuss the philosophy of changing NHTS has the potential to change many downstream modules. Difficult to understand its effects…
The need to investigate the inputs the variables, etc
 

The difficulties found that in the 2017 NHTS there were changes in the variables ….


Therefore, while it has been demonstrated that it is possible to update the NHTS data to a newer version and then re-estimate and run VE, it is a somewhat difficult process that required some imputation in the data to add a variable that is no longer collected in the NHTS, some debugging to isolate estimation failures and their causes, and some grappling with non-standard model object formats in order to understand the reasonableness of the newly estimated models. 

This generally points to some considerations for users and the future development of the VE framework:

1. It is generally feasible that VE could be updated to new versions of the NHTS (e.g., 2017 or 2022 when it becomes available), although rather than the simple re-estimation without re-specification approach taken here it would be preferable to at least respecify models that incorporate 
