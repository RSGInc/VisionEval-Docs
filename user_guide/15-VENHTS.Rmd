...
4. Given the effort required to update the model system with a later version of the NHTS, which, despite the differences discussed here, was still very similar to the 2001 data, it is likely to be difficult for model users to successfully incorporate a local household travel survey. However, it would be reasonable to focus on a more targetted update to individual modules. 

5. The intial comparison of the results from the 2001 and 2017 applications of RSPM suggest (in a very preliminary finding) that updating the estimation data source moderately (as opoosed to significantly) affects the outputs of the model. This effort did not include the more extenstive sensitivity testing and validation that should be considered for the transition to a new estimation dataset.  
...
```{r diagnostics_data_results, eval=T, include=F, message=F, warning=F, comment=F}
knitr::opts_chunk$set(warning = FALSE, message = FALSE, comment = FALSE) 
# Read in the results for both 2001 and 2017. Convert to data.table for performance
library(data.table)
library(gridExtra)
library(ggplot2)
library(scales)
load('./data/resultsnhts2001.rda')
load('./data/resultsnhts2017.rda')
...
compare_plot <- function(params) {
  ...
  df = resultsnhts2017[[paste0(year,".",table)]]
  df2001 = resultsnhts2001[[paste0(year,".",table)]]
  ...
  if(plot_type == 'histogram') {
    ...
    if (nrow(dat) == 2) {
      ...
      bw <- 2 * IQR(dat[value>0,value], na.rm = T) / (max(dat$value, na.rm=T) - min(dat$value,
                                                                                    na.rm=T))^(1/3)
    }
  ...
  if(plot_type == 'bar') {
    ...
    # Initialize plot
    plot <- ggplot(dat, aes(x=NHTS, y = value, lable = value, fill=NHTS)) +
      geom_col() +
...
# Manually set plot settings
compare_vars <- fread('./data/plot_settings_results.csv')
```

```{r plots_results, eval=T, echo=F, out.height="20%", out.width="100%", warning=F}
# chunker <- function(x, n) split(x, sort(x %% (length(x) %/% n)))
chunker <- function(x, n) split(x, ceiling(seq_along(x) / n))
plots_results <- lapply(split(compare_vars, compare_vars$variable), compare_plot)
chunks <- chunker(1:length(plots_results), 6)

for(p in chunks) {
  do.call("grid.arrange", c(plots_results[p], ncol=2))
}
...
# New visualization of percent changes between NHTS years

library(doBy)
library(dplyr)

# Set up data into dfs
results2001_df <- rbindlist(resultsnhts2001, fill=T)
results2017_df <- rbindlist(resultsnhts2017, fill=T)

var_list <- as.list(compare_vars$variable)
  append_list <- list("Bzone")
  var_list_2 <- append(append_list, var_list)
  length(unique(results2001_df$Bzone))
  length(unique(results2017_df$Bzone))

filter_2001 <- as.data.frame(results2001_df[,unlist(var_list_2),with=F])
filter_2017 <- as.data.frame(results2017_df[,unlist(var_list_2),with=F])

# Collapse for variable sums
filter_2001[is.na(filter_2001)] <- 0
    collapse_2001 <- summaryBy(.~ Bzone, FUN = c(sum), data = filter_2001)
filter_2017[is.na(filter_2017)] <- 0
    collapse_2017 <- summaryBy(.~ Bzone, FUN = c(sum), data = filter_2017)

collapse_2001 = subset(collapse_2001, select = -c(Bzone))
    collapse_2001 <- as.data.frame(sapply(collapse_2001, sum))
collapse_2017 = subset(collapse_2017, select = -c(Bzone))
    collapse_2017 <- as.data.frame(sapply(collapse_2017, sum))
    
# Merge and calculate percent change between 2001 and 2017 data 
merged <- cbind(collapse_2001, collapse_2017)
  names(merged)[1] <- "var2001"
  names(merged)[2] <- "var2017"
  merged$Percent_change <- ((merged$var2001-merged$var2017)/merged$var2001)
  merged$Percent_change <- round(merged$Percent_change, digit=4)
  merged<-merged[!(merged$Percent_change==0),]
  merged <- merged[!is.na(merged$Percent_change), ]
      keep <- c("Percent_change")
  merged <- merged[keep]
  merged <- tibble::rownames_to_column(merged, "Var")
  merged$Var = gsub("*.sum", "", merged$Var)
  merged <- merged[order(-merged$Percent_change),]
  merged$num <- 1:nrow(merged)
  
# Use dummy for grouped graphing

  merged$dummy <- 1
    merged$dummy <- ifelse (merged$num>10,2,merged$dummy)
    merged$dummy <- ifelse (merged$num>20,3,merged$dummy)
    merged$dummy <- ifelse (merged$num>30,4,merged$dummy)
    merged$dummy <- ifelse (merged$num>40,5,merged$dummy)
    merged$dummy <- ifelse (merged$num>50,6,merged$dummy)
    merged$dummy <- ifelse (merged$num>60,7,merged$dummy)

# Group and set up plots
  pct_plot <-lapply(unique(merged$dummy), function(d) {
  ggplot(merged %>% filter(dummy == d), aes(x = Var, y = Percent_change)) +
  geom_bar(stat="identity", width = 0.5, fill = "#8da0cb") +
  theme(axis.text.x=element_text(angle=45, hjust=1)) +
  geom_hline(yintercept = 0) +
  labs(title = "NHTS 2001-2017 percent changes", subtitle = "Variables with no change omitted") +
  xlab("Variable") + ylab("") +
...
1. Overal model fit statistics (such as R squared from a linear regression, Rho squared from a logit model, or other statistics such as AIC)
