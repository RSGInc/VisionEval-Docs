...
|                       | PredictHousing              | HouseTypeModel glm                    | Failure                                                                                                                                       | Estimation runs, but results are all the same                   |
...
compare_plot <- function(params) {
  ...
  if(plot_type == 'histogram') {
    # Setup data
    binned <- rbind(data.table(NHTS='2001', value=Hh_df2001[[var]]),
                 data.table(NHTS='2017', value=Hh_df[[var]]))
    binned[!is.finite(value), value := 0]
    
    # Bin widths
    bw <- 4 * IQR(binned$value, na.rm = T) / (max(binned$value, na.rm=T) - min(binned$value, na.rm=T))^(1/3)

    # Initialize plot
    plot <- ggplot(binned, aes(x=value, fill=NHTS)) +
