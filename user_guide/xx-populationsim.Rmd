...
3. **AssignLifeCycle.R** - Assigns household life cycle variable (LIF_CYC) is defined by 2009 NHTS
...
The following subsections are taken from the module documents and describe how each module functions in more detail individually.
...
#### Model Parameter Estimation
This model has just one parameter object, a matrix of the probability that a person in each age group in a specified household type is a worker. It is the same probability matrix used in CreateHouseholds.R created from Census public use microsample (PUMS) data that is compiled by the CreateEstimationDatasets.R script into a R dataset (Hh_df) when the VESimHouseholds package is built.
