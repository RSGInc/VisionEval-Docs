...
1. **VESimHouseholds** package inputs, mechanics, and outputs are described. Within this section is a description of the individual modules used in the package and their contribution to the final household population data. The specific modules are:
   - CreateHouseholds
   - PredictWorkers
   - AssignLifeCycle
   - PredictIncome
...
2. **PopulationSim** inputs, mechanics, and outputs are described for use as a third-party population synthesizer.


>**PLACEHOLDER TEXT???**
Note on the usage of appended data: One paramount consideration when adding population variables is whether the variable *influences* travel behavior or is merely impacted by travel behavior. It is important to understand that additional population data are appended to the population for post-processing analysis, and *does not* influence travel behavior until the models in modules after population synthesis are modified to includes those new population characteristics as explanatory variables. For example, language speaking abilities or persons with disabilities might strongly influence travel behavior. However, including these features in the population data would not affect travel as estimated by VisionEval as the other models in VisionEval do not yet include those characteristics as explanatory variables. A post-processing analysis example would be to append the population data with race and ethnicity features and then to measure pollution impacts on race/ethnicity in different regions.

## A Primer on Population Synthesis
Population synthesis is essentially the process of expanding a small disaggregated data sample (i.e., one record per household) through replication to match aggregated total population counts. The challenge is that the sample data is almost never a perfect sample. When the sample data is expanded to match one variable, it typically will cause the other variables to become poorly fit. To mitigate this, population synthesis iteratively fits each variable until the total error is minimized. An additional challenge in population synthesis is in handling multilevel data with both individual persons and households that contain persons. Matching all target total counts with that many degrees of freedom can be difficult and computationally intensive.

## VESimHouseholds Package
The VESimHouseholds package contains four primary modules:
1. **CreateHouseholds.R** - Synthesizes the base household population data
2. **PredictWorkers.R** - Estimates the number of workers in each household
3. **AssignLifeCycle.R** - Assigns household life cycle variable (LIF_CYC) as defined by 2009 NHTS
4. **PredictIncome.R** - Estimates the household income for each household

These individual modules are run sequentially, starting with CreateHouseholds, each adding relevant information to the base household population data. Some modules have pre-requisite dependencies, such as the AssignLifeCycle module requires the number of household workers to be assigned before it can be run in order to determine whether persons are retired. Below is a general summary table of inputs and outputs for each model.

| Module           | Input files | Datastore Input Fields | Datastore Output Fields |
| ---------------- | ----------- | ----- | ------ |
| CreateHouseholds | VESimHouseholds/inst/extdata/pums\_household.csv<br>VESimHouseholds/inst/extdata/pums\_persons.csv<br>your\_model/inputs/azone\_hh\_pop\_by\_age.csv<br>your\_model/inputs/azone\_hhsize\_targets.csv<br>your\_model/inputs/azone\_gq\_pop\_by\_age.csv | Geo<br>Year<br>Age0to14<br>Age15to19<br>Age20to29<br>Age30to54<br>Age55to64<br>Age65Plus<br>AveHhSize<br>Prop1PerHh<br>GrpAge0to14<br>GrpAge15to19<br>GrpAge20to29<br>GrpAge30to54<br>GrpAge55to64<br>GrpAge65Plus | Azone<br>Marea<br>Age0to14<br>Age15to19<br>Age20to29<br>Age30to54<br>Age55to64<br>Age65Plus<br>HhSize<br>HhType<br>NumHh<br>NumGq<br>HhId |
| PredictWorkers   | your\_model/inputs/azone\_relative\_employment.csv      | Age0to14<br>Age15to19<br>Age20to29<br>Age30to54<br>Age55to64<br>Age65Plus<br>RelEmp15to19<br>RelEmp20to29<br>RelEmp30to54<br>RelEmp55to64<br>RelEmp65Plus<br>HhType<br>Azone    | Wkr15to19<br>Wkr20to29<br>Wkr30to54<br>Wkr55to64<br>Wkr65Plus<br>Workers<br>NumWkr    |
| AssignLifeCycle | None | Age0to14<br>Age15to19<br>Age20to29<br>Age30to54<br>Age55to64<br>Age65Plus<br>Wkr15to19<br>Wkr20to29<br>Wkr30to54<br>Wkr55to64<br>Wkr65Plus<br>HhType     | LifeCycle   |
| PredictIncome    | your\_model/inputs/azone\_per\_cap\_inc.csv  |  Azone<br>HHIncomePC<br>GQIncomePC<br>Azone<br>HhSize<br>HhType<br>Wkr15to19<br>Wkr20to29<br>Wkr30to54<br>Wkr55to64<br>Age65Plus  | Income  |
...
- *group quarters* households (i.e. persons in group quarters such as college dormatories).
...
The base year household model has one parameter object, a probability matrix that a person in each age group is in a household types. This probability matrix is created from Census public use microsample (PUMS) data that is compiled into a R dataframe (Hh_df) when the VESimHouseholds package is built. The data that is supplied with the VESimHouseholds package downloaded from the official VisionEval repository may be used, but it is preferrable to use data for the region being modeled. How this is done is explained in the Build Process documentation **[LINK]**.
...
After the iterations have been completed, the numbers of households by type are rounded to create whole number amounts. Then individual household records are created for each. Below is an example of household frequencies matched to the target totals.
...
| Target Total| 552      | 120       | 312       | 702       | 878       | 420       | 2,984       |
...
Modifying the seed data for local data requires downloading new data, replacing the old, and rebuilding the package (see Build Process[**LINK**]). The target totals are stored in three separate files located in the model's input directory.
...
#### Model Parameter Estimation
This model has just one parameter object, a matrix of the probability that a person in each age group in a specified household type is a worker. It is similar to the probability matrix used in CreateHouseholds.R created from Census public use microsample (PUMS) data that is compiled by the CreateEstimationDatasets.R script into a R dataset (Hh_df) when the VESimHouseholds package is built.

The data that is supplied with the VESimHouseholds package downloaded from the official VisionEval repository may be used, but it is preferrable to use data for the region being modeled. How this is done is explained in the Build Process documentation **[LINK]**.
...
>PLACEHOLDER TEXT
Proposed integration method & notes
